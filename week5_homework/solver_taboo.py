...
# 禁断探索法 (taboo Search)、2-optによる経路改善
def taboo_search(tour, address_list, iteration_num, max_length_taboo):
    n = len(address_list) # 都市の数 
    best_tour = tour[:] # 探索中に見つかった最良の経路を保存
    best_tour_length = calc_length(tour,address_list)
    taboo_list = deque()

    for i in range(iteration_num):
        best_change = float('inf') # その反復で最大の改善量（最小の差）を追跡する変数。非常に大きな値で初期化される。
        ...
        if len(taboo_list) > max_length_taboo: #タブーリストのサイズがmaxを超えた場合、最も古い要素を削除
            taboo_list.popleft()
...
if __name__ == '__main__':
    ...
    iteration_num , max_length_taboo = 50, 5 #  iteration_numは改善を行う回数、max_length_tabooはタブーリストの大きさ
