...
* スタックで書ける探索を再帰で書けるのは、言語処理系が関数呼び出しをスタックで実現しているからで（スタックマシンと言います）、それに便乗しているだけなんです。なので、dfs() -> dfs() -> dfs() -> ...と再帰を繰り返すことで言語処理系の内部でスタックに関数が積まれていくので「見えないところ」でメモリが消費されてるんですね。このメモリ消費が一定量を超えないように関数呼び出しの深さには上限があって、再帰が一定回数を超えるとエラーになる、というしくみです。
自分でスタックを作れば、関数呼び出しの上限を回避できるので、（言語処理系のメモリ上限やOSのメモリ上限にぶつかるまでは）スタックを伸ばしていくことができます。
いずれにせよ、Medium / Large では N と E が大きいので、O(N+E)のアルゴリズムを考えないと動かないです！
* たぶんだけど、Wikipediaのグラフは、1個の巨大なSCCと、たくさんの小さなSCCたち（孤島）からできていて、「渋谷」と「池袋」は巨大な1個のSCCに含まれているので、結局は巨大なSCC内の最長パスをどう探すかという問題に帰着するような気がします。
### 提出コード
