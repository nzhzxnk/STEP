...
static inline void set_flags(my_metadata_t *meta, bool allocated, bool prev_allocated) {
    size_t size = get_size(meta); 
    meta->size_and_flag = size | (allocated ? 1 : 0) | (prev_allocated ? 2 : 0);
}

static inline bool is_prev_allocated(my_metadata_t *meta) {
    return (meta->size_and_flag & 2) != 0;
}
