...
#include <stdlib.h>
#include <string.h>
#define NUM_BINS 10 // 例: ビンの数。サイズクラスに応じて決定
...
typedef struct my_heap_t {
  // 各ビン（サイズクラス）ごとのフリーリストのヘッド
  my_metadata_t *bins[NUM_BINS];
  // 各ビンのリストのダミーヘッド（と必要ならダミーテイル）も配列で持つ
  my_metadata_t dummy_heads[NUM_BINS];
  my_metadata_t dummy_tails[NUM_BINS]; // 双方向リストの場合
} my_heap_t;
