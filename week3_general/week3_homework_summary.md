...
* tokensのリストを変化させずにindexをうまく活用する、dequeを使う別解を思いついたので、calculator_4.pyで実装する予定。
* → ()の処理はできそうだけど、*or/の処理が難しい。indexにこだわらない方が効率がいいかも？　(25/06/12 0:00)
...
* きちんともモジュール化できているか
* 変数名や関数名が抽象的すぎず具体的で、何をしているのか何に使う関数なのかがわかりやすくなっているか？
* 例えばカッコのカウントをする際の変数である'parenthese_count',　カッコが内側にある（ネストされている）順をあらわす'layer'現在注目しているカッコのlayerを表す'now_layer'などの変数名が少し怪しいかと思ったけど、hidehikoさんだったらどんな名前つけるか聞いてみたい。
